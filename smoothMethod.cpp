    ...
    for (int i=0; i<v.size(); i++) {// for each vertex in mesh
    	...
    	if(v[i].onBound == 0){	
    		for(int j= 0; j<v[i].numOfNeighborFace; j++){ // for each vertex in neihgbor face
    			lastV = v[v[i].neighbors[(v[i].numOfNeighborFace+j-1)%v[i].numOfNeighborFace]];
                thisV = v[v[i].neighbors[j]];
                nextV = v[v[i].neighbors[(j+1)%v[i].numOfNeighborFace]];



                    //cout << "lastV thisV nextV " << v[i].neighbors[(v[i].numOfNeighborFace+j-1)%v[i].numOfNeighborFace]
                    //<<" " <<v[i].neighbors[j] <<" " <<v[i].neighbors[(j+1)%v[i].numOfNeighborFace] <<endl;
